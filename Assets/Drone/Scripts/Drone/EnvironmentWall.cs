...
public class EnvironmentWall : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider collider)
    {
        if (collider.transform.tag == "Drone")
        {
            ...
            //droneAgent.AddNegativeReward(droneAgent.drone.Information.currentSpeed * goalReward - 1);
            //droneAgent.AddNegativeReward(droneAgent.drone.Information.currentSpeed * -1 + goalReward * 5);
            //droneAgent.AddNegativeReward(goalReward * 10);
            droneAgent.AddNegativeReward(goalReward);
        }
    }
}